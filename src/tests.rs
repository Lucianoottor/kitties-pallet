...
fn cannot_own_too_many_kitties() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            PalletKitties::create_kitty(RuntimeOrigin::signed(1)),
            Error::<TestRuntime>::TooManyOwnedKitties
        );
    });
}
...
fn transfer_logic_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            ...
            Error::<TestRuntime>::SelfTransfer
        );
...
fn set_price_emits_event() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(PalletKitties::create_kitty(RuntimeOrigin::signed(ALICE)));
        let kitty_id = Kitties::<TestRuntime>::iter_keys().collect::<Vec<_>>()[0];
        assert_ok!(PalletKitties::set_price(RuntimeOrigin::signed(ALICE), kitty_id, 1337));
        // Assert the last event is `PriceSet` event with the correct information.
        System::assert_last_event(
            Event::<TestRuntime>::PriceSet { owner: ALICE, kitty_id, price: 1337 }.into(),
        );
    })
}
...
fn abandon_kitty() {
	new_test_ext().execute_with(|| {
		// Create a kitty to abandon
		assert_ok!(PalletKitties::create_kitty(RuntimeOrigin::signed(ALICE)));
		let kitty_id = Kitties::<TestRuntime>::iter_keys().collect::<Vec<_>>()[0];
		// Verify kitty exists and is owned by ALICE
		assert!(Kitties::<TestRuntime>::contains_key(kitty_id));
		assert_eq!(KittiesOwned::<TestRuntime>::get(ALICE), vec![kitty_id]);
		// Try to abandon the kitty
		assert_ok!(PalletKitties::abandon_kitty(RuntimeOrigin::signed(ALICE), kitty_id));
		// Verify kitty is no longer owned by ALICE
		assert_eq!(KittiesOwned::<TestRuntime>::get(ALICE), vec![]);
	})
}
...
fn breed_kitty() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(PalletKitties::create_kitty(RuntimeOrigin::signed(ALICE)));
		assert_ok!(PalletKitties::create_kitty(RuntimeOrigin::signed(ALICE)));
		let kitty_ids = Kitties::<TestRuntime>::iter_keys().collect::<Vec<_>>();
		// Try to breed the two kitties
		assert_ok!(PalletKitties::breed_kitties(
			RuntimeOrigin::signed(ALICE),
			kitty_ids[0],
			kitty_ids[1]
		));
		// Verify a new kitty has been created
		assert_eq!(KittiesOwned::<TestRuntime>::get(ALICE).len(), 3);
	})
}
